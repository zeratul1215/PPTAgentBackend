...
def edit_pages(edits: list[PageEdit], runtime: ToolRuntime) -> dict[str, Any]:
    ...
    def _run_one_locked(i: int, page: int, demand: str, slot: int | None, use_deck_style: bool) -> dict[str, Any]:
    ...
    def _run_one(i: int) -> dict[str, Any]:
        ...
        with page_job(pid, int(item["slot"])):
            ...
            return _run_one_locked(i, int(page), item["demand"], item["slot"], bool(item.get("use_deck_style")))
    ...
    results_by_index = execute_page_jobs(pid, [(i, int(item["slot"])) for i, item in enumerate(parsed)], _run_one)
